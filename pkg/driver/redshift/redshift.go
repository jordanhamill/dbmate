...
func NewDriver(config dbmate.DriverConfig) dbmate.Driver {
	config.DatabaseURL.Scheme = "postgres"
	return &Driver{
...
func (drv *Driver) openRedshiftDB() (*sql.DB, error) {
	// clone databaseURL
	redshiftURL, err := url.Parse(connectionString(drv.databaseURL))
	if err != nil {
		return nil, err
	}

	// connect to redshift database
	redshiftURL.Path = "postgres"

	return sql.Open("postgres", redshiftURL.String())
}
...
func (drv *Driver) quotedMigrationsTableNameParts(db dbutil.Transaction) (string, string, error) {
	...
	// Differs from Postgres driver here.
	tableNameParts = append([]string{schema}, tableNameParts...)
	quotedNameParts := []string{}
	for _, part := range tableNameParts {
		quoted := fmt.Sprintf("\"%s\"", part)
		quotedNameParts = append(quotedNameParts, quoted)
	}
